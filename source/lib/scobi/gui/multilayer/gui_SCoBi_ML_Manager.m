...
classdef gui_SCoBi_ML_Manager < SCoBiGUIManagers
%GUI_SCO_ML_MANAGER This class implements handles for GUI elements, and 
...
defaultInputFileName = 'default_input-scobi_ml.mat'; 
...
% Constructor
function obj = gui_SCoBi_ML_Manager( handles )            
...
dir_gui_ml = Directories.getInstance.scobi_gui_multilayer;
...
fprintf('SCoBi-ML GUI initialization completed in %.2f seconds\n', t0);
...
groupIDs.sim_settings = [id.panel_sim_settings id.text_sim_mode : id.popup_gnd_cover];
...
groupIDs.sim_inputs       = [id.panel_sim_inputs id.text_sim_name : id.edit_Nr];
...
groupIDs.on_popup_sim_mode = [id.text_Nr id.edit_Nr];
...
% TO-DO: Check if it can be done in the super-class
...
%% GET GLOBAL DIRECTORIES
dir_gui_ml = Directories.getInstance.scobi_gui_multilayer;
dir_input_ml = Directories.getInstance.input_ml;


% Read the last input file, if any
lastInputFileName = [];
if exist( [strcat(dir_gui_ml, '\') obj.lastInputFile], 'file' )
    filename = strcat( dir_gui_ml, '\', obj.lastInputFile);
    load(filename);
...
elseif exist([strcat(dir_input_ml, '/') obj.defaultInputFileName],'file')
...
% function inputStruct = saveGUIToInputFile(obj, filename, pathname )
...
% Number of Fresnel Zones is 1 in SCoBi-ML
inputStruct.Nfz	= 1;
