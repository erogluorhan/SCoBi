...
[filename, pathname] = uigetfile( ...
    {'*_rover_*.bin','SCoBi-saved rover stream data (UBX, SkyTraq, FTX, BINR) (*_rover_*.bin)'; ...
...
%check if RINEX or SCoBi data is requested
if (get(handles.output_type, 'SelectedObject') == handles.out_gogps_binary)
    streams2SCoBibin(filerootIN, filerootOUT, constellations, wait_dlg);
...
if (hObject == handles.out_rinex)
    ...
    if (strcmp(get(handles.data_out_folder, 'String'), '../data/data_SCoBi'))
    ...
    if (strcmp(get(handles.data_out_folder, 'String'), '../data/data_RINEX'))
        set(handles.data_out_folder, 'String', '../data/data_SCoBi');
    end
