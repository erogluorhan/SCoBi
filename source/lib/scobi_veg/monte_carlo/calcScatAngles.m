...
% repmat
pos_Rx_m = repmat(pos_Rx_m', Npart, 1) ;         % Receiver
pos_RxI_m = repmat(pos_RxI_m', Npart, 1) ;       % Image Receiver

pos_Tx_m = repmat(pos_Tx_m', Npart, 1) ;         % Transmitter
pos_TxI_m = repmat(pos_TxI_m', Npart, 1) ;       % Image Transmitter


% Scatteting directions and distances to Transmitter / Image Transmitter
ki = pos_Tx_m - pP ;                  % vector from particle to antenna
ri = sqrt(sum(ki' .^ 2))' ;     % distance from particle to antenna

kiI = pos_TxI_m - pP ;                    % vector from particle to image antenna
riI = sqrt(sum(kiI' .^ 2))' ;       % distance from particle to image antenna


% Scatteting directions and distances to receiver/image reciever
ko = pos_Rx_m - pP ;                  % vector from particle to antenna
ro = sqrt(sum(ko' .^ 2))' ;     % distance from particle to antenna
ko = ko ./ repmat(ro, 1, 3) ;   % unit vector

koI = pos_RxI_m - pP ;                    % vector from particle to image antenna
