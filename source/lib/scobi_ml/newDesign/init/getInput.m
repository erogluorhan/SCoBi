...
[num, ~, ~] = xlsread( strcat( dir_input_dyn, '\', inputFile ),1);

[rows, cols] = size(num);

ind = 1;

DOYs = num(:, ind);   % Day-of-year
DOYs( isnan(DOYs) ) = [];
ind = ind + 1;
...
% TO-DO: 4 stands for SM5, SM10, SM15, SM20. Generalize it. 
VSM_list_cm3cm3 = num(:, ind : ind + 3 );    % Theta probe 
...
% Multilayer is only available for Homogenous vegetation if there is
% vegetation cover
veg_method_id = Constants.id_veg_hom;

% Nfz = 1 enough for Specular 
Nfz = 1;

% Initialize Simulation Parameters
SimParams.getInstance.initialize(sim_name, campaign, campaign_date, ...
    plot, veg_method_id, [], [], [], Nfz );
...
simulator = getStringFromXML(xDoc, ConstantNames.set_simulator);        % 
...
SimSettings.getInstance.initialize( simulator_id, [], gnd_cover_id, ...
    write_attenuation, calc_direct_term, calc_specular_term, ...
    calc_diffuse_term );
